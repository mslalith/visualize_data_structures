...
class _BubbleSortVisualizerState extends State<BubbleSortVisualizer>
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: Selector<BubbleSortProvider, List<int>>(
  ...
  Widget _buildArrayNodes() {
    return Column(
      ...
      children: <Widget>[
        provider.state == BubbleSortState.swap
            ? _buildAnimatedNodes()
            : _buildNodes(),
        _buildIndexes(),
        _buildIJ(),
  ...
  String _getIJText(index) {
    if (provider.jIndex == index && provider.jIndex + 1 == index) return 'i j';

    if (provider.jIndex == index) return 'i';

    if (provider.jIndex + 1 == index) return 'j';
